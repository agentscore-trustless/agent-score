 ...
 * @dev Implements the IReceiver interface via ReceiverTemplate to ensure only the Chainlink Forwarder can submit assertions.
 *      Based on specifications for the AgentScore protocol.
 */
contract AgentScoreRegistry is ERC721, AccessControl, ReceiverTemplate {
    ...
    /**
     * @notice Constructor to initialize the contract with an admin and forwarder.
     * @param admin The address to be granted DEFAULT_ADMIN_ROLE.
     * @param forwarder The address of the Chainlink CRE Forwarder on this chain.
     */
    constructor(
        address admin,
        address forwarder,
    ...
    ) ERC721(name_, symbol_) ReceiverTemplate(forwarder) {
    ...
    function supportsInterface(
        ...
        public
        view
        override(ERC721, AccessControl, ReceiverTemplate)
        returns (bool)
    {
    ...
    /**
     * @notice Processes the incoming report securely dispatched by the Chainlink DON Forwarder.
     * @dev Automatically decodes the structured ABI payload to update agent scores.
     * @param report The raw ABI encoded report byte array.
     */
    function _processReport(bytes calldata report) internal override {
        // Decode the incoming report generated by the compute capability
        (uint256 tokenId, int256 scoreDelta, bytes memory data) = abi.decode(
            report,
            (uint256, int256, bytes)
        );
